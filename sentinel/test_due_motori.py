...
try:
    print("Test dei gradi del servo sui canali 0 e 1...")
    ...
    for duty in range(duty_min, duty_max, increment):  # Incrementi di 50
        pca.channels[0].duty_cycle = duty  # Motore sul canale 0
        pca.channels[1].duty_cycle = duty  # Motore sul canale 1
    ...
    # Posiziona i motori al centro
    pca.channels[0].duty_cycle = 0x1999  # Circa 7.5% duty cycle (angolo centrale)
    pca.channels[1].duty_cycle = 0x1999  # Circa 7.5% duty cycle (angolo centrale)
    ...
    print("Spegnimento dei canali 0 e 1...")
